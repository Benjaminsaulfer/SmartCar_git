    ...
    if(motor_flag == 0){//如果电机关闭
      switch(ArrowPos){
         ...
         case 176://电机使能
            ...
            if(gpio_get_level(P20_3) == 0){
              motor_flag = 1;
              ips200_Printf(0,176,(ips200_font_size_enum)0,"motor:  %d",motor_flag);// 电机使能
            }
            break;
      }
    }else{//如果电机打开
      if(gpio_get_level(P20_0) == 0)motor_flag = 0;
         if(gpio_get_level(P20_3) == 0){
          motor_flag = 1;
          ips200_Printf(0,176,(ips200_font_size_enum)0,"motor:  %d ",(uint32_t)motor_flag);// 电机使能
      }
    }
}
...
{     
    ...
    //PID初始化
    PID_init(&PID_Steering,900,0,200,0);//直线PID
    PID_init(&PID_Steering_turn,2600,0,2,0);//直线PID
    ...
    while(true)
    {
        ...
        if(motor_flag == 0){//如果电机没有打开允许屏幕运行
            ...
            ips200_Printf(50,176,(ips200_font_size_enum)0,"%d",(uint32_t)motor_flag);// 电机使能
        ...
        if(REC.LL <= 3 && REC.RR <= 3){
          Steering_FeedBack(&PID_Steering,Sum_of_Dif_near(REC.L,REC.R));//直线转向环
          gpio_set_level(P19_4,0);
        }else{
          Steering_FeedBack(&PID_Steering_turn,Sum_of_Dif_near(REC.LL,REC.RR));//弯道转向环
