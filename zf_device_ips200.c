...
void ips200_draw_big_point (uint16 x, uint16 y, const uint16 color)
{
    // 如果程序在输出了断言信息 并且提示出错位置在这里
    // 那么一般是屏幕显示的时候超过屏幕分辨率范围了
    if(x <= 1)x = 1;
    ...
    ips200_set_region(x, y, x, y);
    ips200_write_16bit_data(color);
    ips200_set_region(x+1, y, x, y);
    ips200_write_16bit_data(color);
    ips200_set_region(x, y+1, x, y);
    ips200_write_16bit_data(color);
    ips200_set_region(x-1, y, x, y);
    ips200_write_16bit_data(color);
    ips200_set_region(x, y-1, x, y);
...
void ips200_draw_rectangle(uint16 x1, uint16 y1,uint16 x2, uint16 y2, const uint16 color){
  ips200_draw_line(x1,y1,x2,y1,color);
  ips200_draw_line(x2,y1,x2,y2,color);
  ips200_draw_line(x2,y2,x1,y2,color);
  ips200_draw_line(x1,y2,x1,y1,color);
}
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IPS200 画线
...
void ips200_show_binary_map(uint16 x, uint16 y, const uint8 *image){
    ...
    for(j = 0; j < 120; j ++)
    {
        ...
        ips200_write_16bit_data_array(data_buffer, 188);
    }
    IPS200_CS(1);//关闭片选
}
...
void ips200_Printf(int16_t X, int16_t Y,ips200_font_size_enum FontSize,const char *format, ...)
{
	char String[256];
	va_list arg;
	va_start(arg, format);
